...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addSubcommand((sub) =>
      sub
        ...
        .addStringOption((option) =>
          option
            .setName("username")
            .setDescription("Tên tài khoản TikTok")
            .setRequired(true)
        )
    )
    .addSubcommand((sub) =>
    ...
    .addSubcommand((sub) =>
      sub.setName("trending").setDescription("Xem video TikTok thịnh hành")
    )
    .addSubcommand((sub) =>
      sub
        .setName("post")
  ...
  async execute(interaction) {
    ...
    if (sub === "info") {
      ...
      await interaction.reply({
        content: `🔎 Đang tìm kiếm người dùng TikTok...`,
        ephemeral: false,
      });
      try {
        ...
        const embed = new EmbedBuilder()
          ...
          .addFields(
            ...
            {
              name: "Theo dõi",
              value: stats.followerCount.toLocaleString("vi-VN"),
              inline: true,
            },
            {
              name: "Đang theo dõi",
              value: stats.followingCount.toLocaleString("vi-VN"),
              inline: true,
            },
            {
              name: "Lượt tim",
              value: stats.heartCount.toLocaleString("vi-VN"),
              inline: true,
            }
          )
          .setColor(0xff0050);
        ...
        console.error(err);
        await interaction.editReply({
          content: "❌ Không thể tìm thấy người dùng hoặc xảy ra lỗi.",
        });
      }
    }
    ...
    if (sub === "video") {
      ...
      try {
        ...
        const videoStream = await axios.get(data.play, {
          responseType: "stream",
        });
        ...
        const embed = new EmbedBuilder()
          ...
          .addFields(
            {
              name: "❤️ Lượt tim",
              value: data.digg_count.toLocaleString(),
              inline: true,
            },
            {
              name: "💬 Bình luận",
              value: data.comment_count.toLocaleString(),
              inline: true,
            },
            {
              name: "🔁 Chia sẻ",
              value: data.share_count.toLocaleString(),
              inline: true,
            },
            {
              name: "⬇️ Tải xuống",
              value: data.download_count.toLocaleString(),
              inline: true,
            },
    ...
    if (sub === "music") {
      ...
      await interaction.reply({
        content: `🔍 Đang tìm kiếm video TikTok với từ khóa: ${keyword}`,
        ephemeral: true,
      });
      try {
        const res = await axios.get(
    ...
    if (sub === "trending") {
      await interaction.reply({
        content: `📈 Đang tải danh sách video thịnh hành TikTok...`,
        ephemeral: false,
      });
      try {
        const res = await axios.get(
        ...
        console.error(err);
        await interaction.editReply({
          content: "❌ Không thể lấy danh sách thịnh hành.",
        });
      }
    }
